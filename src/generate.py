#!/usr/bin/env python
"""
Generate synthetic time series data for training/testing models.
"""
...
def _generate_data() -> dict[str, np.ndarray]:
	...
	print(df.head())
	df.to_csv(PROJECT_ROOT / "data" / "data_signals.csv", index=False)

	# Return
	logging.info(f"Generated synthetic data and saved to {PROJECT_ROOT / 'data' / 'data_signals.csv'}")
	return inputs


def _plot(inputs: dict[str, np.ndarray]) -> None:
	...
	# Save
	plt.tight_layout()
	plt.savefig(ARTIFACT_PATH / "data_signals.png", dpi=800)
	logging.info(f"Saved data signals plot to {ARTIFACT_PATH / 'data_signals.png'}")


if __name__ == "__main__":
	# Setup logging
	logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
	inputs = _generate_data()
	_plot(inputs)
